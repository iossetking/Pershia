...
import { XMarkIcon, ChevronLeftIcon, TrashIcon } from '@heroicons/react/24/outline';
...
import { useOutfits, useDeleteOutfit } from '@/features/outfits/hooks/useOutfits';
      ...
      <div className="bg-white rounded-3xl w-full max-w-2xl shadow-2xl overflow-hidden flex flex-col md:flex-row" onClick={e => e.stopPropagation()} style={{ maxHeight: '90vh' }}>
        ...
        <div className="md:w-7/12 p-6 flex flex-col gap-4 overflow-y-auto">
          <div className="flex justify-between items-start">
            ...
            <button onClick={onClose} className="text-gray-400 hover:text-gray-700 transition-colors"><XMarkIcon className="w-5 h-5" /></button>
          </div>
...
function OutfitDetailModal({ outfit, garments, onClose, onDelete }: { outfit: Outfit; garments: Garment[]; onClose: () => void; onDelete: () => void }) {
    ...
    return (
      <div className="mt-6 flex flex-wrap gap-4">
        {Array.from({ length: 3 }).map((_, i) => (
          <div key={i} className="w-[160px] md:w-[250px] rounded-2xl bg-gray-100 animate-pulse" style={{ height: 240 }} />
        ))}
      </div>
    );
  }
  ...
  return (
    <div className="mt-6 pb-28">
      <div className="flex flex-wrap gap-4 justify-start">
        {outfits.map(outfit => (
          <div key={outfit.outfit_id} onClick={() => setSelectedOutfit(outfit)}
      ...
      {selectedOutfit && (
        <OutfitDetailModal
          ...
          onDelete={() => handleDelete(selectedOutfit.outfit_id)}
        />
      )}
    </div>
  );
