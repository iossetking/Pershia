...
export default function Canvas({ canvasItems, setCanvasItems, maxZIndex, setMaxZIndex, onRemoveItem }: CanvasProps) {
  const [activeItemId, setActiveItemId] = useState<string | null>(null);
  const { data: garments = [] } = useGarments();

  const garmentsMap = useMemo(() => {
    return Object.fromEntries(garments.map(g => [g.garment_id, g]));
  }, [garments]);
  ...
  return (
    <div className="w-full lg:w-[40%] h-[60vh] lg:h-full flex justify-center items-center bg-white rounded-3xl shadow-sm border border-gray-100 p-4 md:p-6 overflow-hidden shrink-0">
      <div
        ...
        onPointerDown={(e) => {
          if ((e.target as HTMLElement).id === 'design-canvas') setActiveItemId(null);
        }}
      >
        ...
        {canvasItems.map((canvasItem) => {
          const garment = garmentsMap[canvasItem.itemId];
          ...
          return (
            ...
            >
              <div className="relative w-full h-full" onPointerDown={() => setActiveItemId(canvasItem.instanceId)}>
                <img
                ...
                />
                <button
                  onPointerDown={(e) => { e.stopPropagation(); onRemoveItem(canvasItem.instanceId); }}
                  className={`absolute -top-3 -right-3 bg-red-500 hover:bg-red-600 text-white p-1.5 rounded-full shadow-lg transition-opacity duration-200 z-50
                    ${activeItemId === canvasItem.instanceId ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}
                ...
                <div className={`absolute -inset-1 border-2 rounded-lg pointer-events-none transition-colors duration-200
                  ${activeItemId === canvasItem.instanceId ? 'border-gray-400/40' : 'border-transparent'}
